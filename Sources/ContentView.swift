...
struct ContentView: View {
    ...
    private func tmuxWorkspacePaneWindowOverlayState(for window: NSWindow) -> TmuxWorkspacePaneOverlayRenderState? {
        ...
        let shouldShowActivePaneBorder = resolvedActivePaneBorderColorHex != nil &&
            !fileExplorerState.rightSidebarOwnsInputFocus &&
    ...
    private func shouldScheduleTmuxWorkspacePaneWindowOverlayGeometryRefresh(in window: NSWindow) -> Bool {
        if TmuxOverlayExperimentSettings.target().usesWorkspacePaneOverlay {
            return true
        }
        if WindowTmuxWorkspacePaneOverlayController.controller(for: window, createIfNeeded: false)?.hasRenderedState == true {
            return true
        }
        guard WorkspaceTabColorSettings.normalizedHex(activePaneBorderColorHex) != nil,
              !fileExplorerState.rightSidebarOwnsInputFocus,
              let workspace = tabManager.selectedWorkspace,
              workspace.bonsplitController.allPaneIds.count > 1 else {
            return false
        }
        return true
    }
